function showInfo(e, t) {
    allRows = _.sortBy(t.sheets("Completed Detailed Data").all(), "Department"), filterByDatatype(pageType)
}

function filterByDatatype(e) {
    clearCards(), updateCards(allRows, [buildDatatypeFilter(e)])
}
...
function updateCards(e, t) {
    ...
    _.chain(e).filter(function(e) {
        ...
        e.free = e["Data is freely available online"], e.machine = e["Data is machine readable"], e.context = e["Context is provided"], e.bulk = e["Available in bulk"], e.fresh = e["Up-to-date"], e.incident = e["Incident-level data"], e.freeCaption = captions.free[e.free], e.machineCaption = captions.machine[e.machine], e.contextCaption = captions.context[e.context], e.bulkCaption = captions.bulk[e.bulk], e.freshCaption = captions.fresh[e.fresh], e.incidentCaption = captions.incident[e.incident], e.free = e["Data is freely available online"], e.machine = e["Data is machine readable"], e.context = e["Context is provided"], e.bulk = e["Available in bulk"], e.fresh = e["Up-to-date"], e.incident = e["Incident-level data"], e.freeCaption = captions.free[e.free], e.machineCaption = captions.machine[e.machine], e.contextCaption = captions.context[e.context], e.bulkCaption = captions.bulk[e.bulk], e.freshCaption = captions.fresh[e.fresh], e.incidentCaption = captions.incident[e.incident];
        var t = n(e);
        $("#cards").append(t), $('[data-toggle="tooltip"]').tooltip()
    })
}
$(document).ready(function() {
    Tabletop.init({
        key: "1hQwcAaAvysI7A6UyWz5l_IQUhEiFysMVK5gNrfk6DvA",
        callback: showInfo,
        parseNumbers: !0
    })
});
